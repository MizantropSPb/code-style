...
### Пустые строчки
 - Любые, логические связаные, порты необходимо отделять пустой строчкой. Под `логические связаные порты` понимается совокупность сигналов, которые имеют одинаковое назначение или цель.
...
```
 ...
 - В сложных выражениях предпочтильно явно писать все условия. `(data_enable != 1'h1)` вместо `(!data_enable)`
...
## Комментарии
 ...
 - Комментарии на английском языке пишут только носители языка. Остальные довольствуются русским в кодировке win. Желающие могут писать очевдные коментарии на английском.
 - Необходимо добавлять расшифровку акронимов и сокращений в названих. В данном случае допустимо писать на английском.
 - Если модуль реализует математический алгоритм то заголовок модуля должен содержать коментарий с математическим описанием. Кроме того, необходимо указать источника алгоритма (книга, статья, рисунок и т.п.).
...
```
  ...
  - начальная иницализация всех тригеров производится в момент их описания
