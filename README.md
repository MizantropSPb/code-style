...
Основной упор сделан на синтезируемое подмножество языка. Инициализация и сброс оринтированы на применение в FPGA.

## Общие вопросы

## Отступы и пробелы
 - Отступ - 2 пробела. Табы (`\t`) запрещены.
 - Отступ используются для обозначения уровня вложенности.
Пример:
```systemverilog
if (a == cons)
  b <= c;
...
## Выравнивание
 - Выравнивание делается всегда. Исключений нет.
 - Выравнивание производится с помощью пробелов. Знаки табуляции (`\t`) запрещены.
...
### Логические выражения
  ...
  - Широкое использвание дополнительных скобок приветсвуется. 
...
```
 ...
 - В сложных выражениях рекомендуется разбивать строку условий на несколько, с соответвующим, дополнительным выравниванием.
...
### `if-else` вместе с `begin/end`
  - Вложенный if всегда обрамляется `begin/end`
...
Для облегчения чтения (и проверки/написания) допускается (и в большинстве случаев рекомендуется) писать в две строки:
```systemverilog
...
### `case`
 ...
 - Если `case` целиком не умещается на одно экране то конец каждого блока маркируется комментарием содержащим копию условия.  
...
Обратите внимание на наличие выравнивания.
...
## Комментарии
 ...
 - Коментарий должен отвечать на вопрос "зачем это сделано". На вопрос "как это сделано" отвечает код.
 - Комментарии на английском языке пишут только носители языка. Остальные довольствуются русским в кодировке win. Желающие, могут дублировать коментарии на русском англоязычным вариантом. Немного юмора никогда не помешает.
 - Необходимо добавлять расшифровку акронимов и сокращений в названых. В данном случае, допустимо писать на английском.
 - Если модуль реализует математический алгоритм, то заголовок модуля должен содержать комментарий с математическим описанием. Кроме того, необходимо указать источник алгоритма (книга, статья, рисунок и т.п.).
...
always_ff @(posedge iclk)
  if (irst)
    ...
    if (pkt_valid && pkt_eop)
      ...
      if (pkt_valid)
        pkt_word_cnt <= pkt_word_cnt + 1'd1;
```
```systemverilog
module 
```
...
- Стиль названия переменных, функций, тасков и модулей - `snake_case`, т.е. слова или префиксы разделяются `_`, всё пишется маленькими буквами.
...
- Следует придерживаться общей схемы наименования переменных предназначенных для однотипных задач. Это позволит заначительно быстрее разбираться в дизайне.

### Предлагаются следующие модификаторы (суффиксы для наименований). Список является открытым и требует дополнений.
- en наличие сигнала на шине. Должен обрабатываться на каждом такте.
- stb строб. Исползуется если необходимо подтчеркунуть поведение сигнала.
- req запрос. В общем случае используется как уровень. Сбрасывается по наличию ответа (ack). Может быть дополнен модификатором stb (один запрос = один такт).
- ack подтверждение запроса. В общем случае испльзуется как уровень. Сбрасывается после сброса запроса (req). Может быть дополнен модификатором stb (одно подтверждение = один такт).
- toggle тригер. Меняет состояние на противоположное по событию. 
- Название переменной строистся по следующим образром: блок (задача и т.п.) "придающая смысл" переменной, содержимое переменной, модификаторы.
  Пример:
  подключение к flesh памяти
  flash_addr
  flash_wr_data
  flash_wr_en
  flash_rd_data
...
- Большими буквами (так же, через подчеркивание `_`) пишутся только константы, параметры, дефайны, состояния FSM
...
Есть два типа ресета (сброса):
 ...
 - В большинстве случаев модули используют один клок. Следует, по возможности, избегать использования более одного клока в модуле. Переход между клоковыми доменами необходимо делать как можно выше по иерархии в рамках отдельного модуля. 
...
В большинстве случаев необходимо использвать автомат Мура. Для описания используется следующая схема:
